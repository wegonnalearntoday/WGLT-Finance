...
function playScenarioFoundationScenario(picked, category){
  ...
  const resultTitle = category === 'financial'
...
function deriveCategoryAwarePreviewNote(cfg, data){
  ...
  if(category === 'financial'){
    if(cfg.previewJob === 'manager'){
      if(clientDelta > 0 || payDelta > 0) return 'Your money choice gave the team more breathing room, and your leadership results improved at work.';
      if(clientDelta < 0 || payDelta < 0) return 'Financial pressure followed you into management, and the team felt it in the weekly results.';
      return 'No major team shift this time, but your money habits still shape your margin for error.';
    }
    if(cfg.previewJob === 'entrepreneur'){
      if(clientDelta > 0 || payDelta > 0) return 'This money choice strengthened your cash cushion, so your business could move with more confidence.';
      if(clientDelta < 0 || payDelta < 0) return 'This financial hit tightened cash flow, and the business lost some breathing room.';
      return 'No big business swing this time, but money habits still shape what you can risk next.';
    }
...
function buildCategoryAwareGenericPreview(cfg, choice, picked){
  if(!cfg || !choice || !picked) return null;
  const category = String((picked && picked.category) || '');
  if(category !== 'financial' && category !== 'opportunity') return null;
  ...
  return {
    ...
    pay_delta_pct,
    client_delta,
    note: deriveCategoryAwarePreviewNote(cfg, { category, outcome, pay_delta_pct, client_delta })
  };
}
