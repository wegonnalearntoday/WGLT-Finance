...
let state = {
  ...
  teacherEliteOn: false,
  teacherControlOn: false,
  teacherManualWeekThemes: {},
  scenarioPlan: {},
  scenarioCoverage: { usedScenarioIds: [], usedTags: [], planMode: 'random' }
};
const JOBS = {
  lemon: {
...
const ELITE_WEEK_THEMES = [
  { key:'spend', label:'Needs vs. Wants Week', detail:'Students practice using money carefully and spotting wants vs. needs.' },
  { key:'save', label:'Saving Week', detail:'Students focus on saving for a future goal.' },
  { key:'share', label:'Sharing Week', detail:'Students think about generosity and helping others.' },
  { key:'mixed', label:'Budget Balance Week', detail:'Students compare spending, saving, and sharing choices together.' },
  { key:'reallife', label:'Real-Life Event Week', detail:'Students work through a surprise or honest-choice situation.' },
  { key:'spend', label:'Smart Spending Week', detail:'Students talk through purchases and tradeoffs.' },
  { key:'save', label:'Goal Push Week', detail:'Students make choices that move them closer to a goal.' },
  { key:'share', label:'Community Week', detail:'Students connect money choices to helping other people.' },
  { key:'planning', label:'Planning Ahead Week', detail:'Students balance work, school, and future choices.' },
  { key:'spend', label:'Treat or Wait Week', detail:'Students decide whether fun now is worth it.' },
  { key:'future', label:'Future You Week', detail:'Students think about choices that pay off later.' },
  { key:'review', label:'Review Week', detail:'Students pull together spending, saving, and sharing ideas.' }
];
const ELITE_THEME_KEYWORDS = {
  spend: ['spend', 'treat', 'buy', 'needs vs wants'],
  save: ['save', 'goal', 'future'],
  share: ['share', 'help', 'community'],
  mixed: ['balance', 'mix', 'split'],
  reallife: ['surprise', 'real life', 'honesty', 'problem solving'],
  planning: ['plan', 'school', 'schedule'],
  future: ['future', 'next week', 'long-term'],
  review: ['review', 'all together']
};
function slugifyScenarioText(str){
  return String(str || '').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'');
}
function ensureEliteState(){
  if(typeof state.teacherEliteOn !== 'boolean') state.teacherEliteOn = false;
  if(typeof state.teacherControlOn !== 'boolean') state.teacherControlOn = false;
  if(!state.teacherManualWeekThemes || typeof state.teacherManualWeekThemes !== 'object') state.teacherManualWeekThemes = {};
  if(!state.scenarioPlan || typeof state.scenarioPlan !== 'object') state.scenarioPlan = {};
  if(!state.scenarioCoverage || typeof state.scenarioCoverage !== 'object') state.scenarioCoverage = { usedScenarioIds: [], usedTags: [], planMode: 'random' };
  if(!Array.isArray(state.scenarioCoverage.usedScenarioIds)) state.scenarioCoverage.usedScenarioIds = [];
  if(!Array.isArray(state.scenarioCoverage.usedTags)) state.scenarioCoverage.usedTags = [];
}
function getEliteThemeForWeek(week){
  return ELITE_WEEK_THEMES[(Math.max(1, week) - 1) % ELITE_WEEK_THEMES.length];
}
function getTeacherControlledThemeForWeek(week){
  ensureEliteState();
  const key = state.teacherManualWeekThemes && state.teacherManualWeekThemes[String(week)];
  if(!key || key === 'random') return null;
  return ELITE_WEEK_THEMES.find(t => t.key === key) || { key, label: key.charAt(0).toUpperCase() + key.slice(1) + ' Week', detail: 'Teacher-selected week focus.' };
}
function getWeekThemeForDisplay(week){
  return getTeacherControlledThemeForWeek(week) || getEliteThemeForWeek(week);
}
function classifyScenarioType(scenario){
  if(!scenario) return ['review'];
  const tags = new Set();
  const text = ((scenario.scenario || '') + ' ' + (scenario.question || '')).toLowerCase();
  const options = Array.isArray(scenario.options) ? scenario.options : [];
  let spendCount = 0, saveCount = 0, shareCount = 0;
  options.forEach(opt => {
    if((opt.spend || 0) < 0 || /buy|treat|snack|spend/.test((opt.text || '') + ' ' + (opt.effect || ''))) spendCount++;
    if((opt.save || 0) > 0 || /save|goal|future/.test((opt.text || '') + ' ' + (opt.effect || ''))) saveCount++;
    if((opt.share || 0) > 0 || /share|donate|gift|help/.test((opt.text || '') + ' ' + (opt.effect || ''))) shareCount++;
  });
  if(spendCount) tags.add('spend');
  if(saveCount) tags.add('save');
  if(shareCount) tags.add('share');
  if((spendCount && saveCount) || (saveCount && shareCount) || (spendCount && shareCount)) tags.add('mixed');
  if(/school|weekend|homework|balance|plan|usual chores|keep working/.test(text)) tags.add('planning');
  if(/friend|borrow|found|nobody is around|nobody saw|wrong store|ran out|rain|hot|hole in|gift card|honest|surprise|uh oh/.test(text)) tags.add('reallife');
  if(/goal|final push|so close|future|next week|final week|last week/.test(text)) tags.add('future');
  if(/final week|last week/.test(text)) tags.add('review');
  if(!tags.size) tags.add('mixed');
  return Array.from(tags);
}
function cloneScenarioForWeek(scenario, currentWeek){
  const cloned = JSON.parse(JSON.stringify(scenario));
  const originalWeek = Number(scenario.week || currentWeek);
  const weekShift = currentWeek - originalWeek;
  cloned.originalWeek = originalWeek;
  cloned.week = currentWeek;
  cloned._scenarioId = scenario._scenarioId || slugifyScenarioText((scenario.scenario || '') + '-' + originalWeek);
  cloned._tags = scenario._tags || classifyScenarioType(scenario);
  if(Array.isArray(cloned.options)){
    cloned.options = cloned.options.map(opt => {
      const optionClone = Object.assign({}, opt);
      if(opt.consequence){
        const offset = Math.max(1, Number(opt.consequence.week || (originalWeek + 1)) - originalWeek);
        optionClone.consequence = Object.assign({}, opt.consequence, { week: Math.min(TOTAL_WEEKS, currentWeek + offset) });
      }
      return optionClone;
    });
  }
...
function getScenarioPoolForJob(jobKey){
  const job = JOBS[jobKey];
  if(!job || !Array.isArray(job.scenarios)) return [];
  return job.scenarios.map((scenario, idx) => {
    if(!scenario._scenarioId) scenario._scenarioId = `${jobKey}-${idx+1}-${slugifyScenarioText(scenario.scenario || 'scenario')}`;
    if(!scenario._tags) scenario._tags = classifyScenarioType(scenario);
    return scenario;
  });
}
...
function buildScenarioPlanIfNeeded(){
  ensureEliteState();
  if(!state.job) return;
  const mode = state.teacherControlOn ? 'manual' : (state.teacherEliteOn ? 'elite' : 'random');
  const existingWeeks = Object.keys(state.scenarioPlan || {}).length;
  if(existingWeeks >= TOTAL_WEEKS && state.scenarioCoverage.planMode === mode) return;
  const pool = getScenarioPoolForJob(state.job).slice();
  const usedIds = new Set();
  const usedTags = [];
  const plan = {};
  for(let week = 1; week <= TOTAL_WEEKS; week++){
    const theme = getTeacherControlledThemeForWeek(week) || getEliteThemeForWeek(week);
    let available = pool.filter(s => !usedIds.has(s._scenarioId));
    if(state.teacherControlOn){
      const manualKey = (state.teacherManualWeekThemes || {})[String(week)];
      if(manualKey && manualKey !== 'random'){
        let forced = available.filter(s => (s._tags || []).includes(manualKey));
        if(!forced.length) forced = pool.filter(s => (s._tags || []).includes(manualKey));
        if(forced.length) available = forced;
      }
    } else if(state.teacherEliteOn){
      let themed = available.filter(s => (s._tags || []).includes(theme.key));
      if(!themed.length && theme.key === 'review') themed = available.filter(s => (s._tags || []).includes('mixed') || (s._tags || []).includes('future'));
      if(!themed.length && theme.key === 'future') themed = available.filter(s => (s._tags || []).includes('save') || (s._tags || []).includes('planning'));
      if(!themed.length && theme.key === 'planning') themed = available.filter(s => (s._tags || []).includes('reallife') || (s._tags || []).includes('mixed'));
      if(themed.length) available = themed;
    }
    if(!available.length) available = pool.slice();
    const chosen = randomPick(available);
    usedIds.add(chosen._scenarioId);
    (chosen._tags || []).forEach(tag => { if(!usedTags.includes(tag)) usedTags.push(tag); });
    plan[week] = cloneScenarioForWeek(chosen, week);
    plan[week]._plannedTheme = theme.key;
    plan[week]._plannedThemeLabel = theme.label;
  }
  state.scenarioPlan = plan;
  state.scenarioCoverage = { usedScenarioIds: Array.from(usedIds), usedTags, planMode: mode };
}
function getScenarioForWeek(week){
  ensureEliteState();
  buildScenarioPlanIfNeeded();
  return state.scenarioPlan[String(week)] || state.scenarioPlan[week] || null;
}
function getCoverageStatusMarkup(){
  ensureEliteState();
  const usedTags = state.scenarioCoverage.usedTags || [];
  const targetTags = state.teacherEliteOn ? ELITE_WEEK_THEMES.map(t => t.key) : ['spend','save','share','mixed','reallife','planning','future','review'];
  const uniqueTargets = [...new Set(targetTags)];
  return uniqueTargets.map(tag => {
    const hit = usedTags.includes(tag);
    const label = (ELITE_THEME_KEYWORDS[tag] && ELITE_THEME_KEYWORDS[tag][0]) ? ELITE_THEME_KEYWORDS[tag][0] : tag;
    return `<span style="display:inline-flex;align-items:center;gap:6px;padding:4px 10px;border-radius:999px;background:${hit ? 'rgba(76,175,80,.18)' : 'rgba(255,255,255,.1)'};border:1px solid ${hit ? 'rgba(76,175,80,.45)' : 'rgba(255,255,255,.14)'};margin:4px 6px 0 0">${hit ? '✅' : '🟡'} ${label}</span>`;
  }).join('');
}
function getTeacherControlTargetWeek(){
  const baseWeek = state.week || 1;
  if(state.gameActive && state.week > 0 && !state.choiceMade) return state.week;
  return Math.min(TOTAL_WEEKS, Math.max(1, baseWeek + (state.choiceMade ? 1 : 0)));
}
function updateTeacherControlUI(){
  ensureEliteState();
  const box = document.getElementById('teacher-control-box');
  const btn = document.getElementById('teacher-control-toggle');
  const info = document.getElementById('teacher-control-info');
  if(btn) btn.textContent = state.teacherControlOn ? '🎛️ Teacher Control: ON' : '🎛️ Teacher Control: OFF';
  if(!box || !info) return;
  if(!isTeacherJrMode()){
  ...
  box.style.display = '';
  const targetWeek = getTeacherControlTargetWeek();
  const manualKey = state.teacherManualWeekThemes && state.teacherManualWeekThemes[String(targetWeek)];
  const manualTheme = getTeacherControlledThemeForWeek(targetWeek);
  info.innerHTML = state.teacherControlOn
    ? `<b>Teacher picks the lesson focus.</b> Buttons now control <b>Week ${targetWeek}</b>${state.gameActive && state.week === targetWeek && !state.choiceMade ? ' right now' : ' next'}. Current pick: <b>${manualTheme ? manualTheme.label : (manualKey === 'random' ? 'Random Mix' : 'Not chosen yet')}</b>.`
    : `Turn this on if you want to choose what the class learns for <b>Week ${targetWeek}</b>. Leave it off to use ${state.teacherEliteOn ? 'the guided 12-week elite path' : 'the fully random mix'}.`;
}
function updateTeacherEliteUI(){
  ensureEliteState();
  const btn = document.getElementById('teacher-elite-toggle');
  const box = document.getElementById('teacher-elite-info');
  if(btn) btn.textContent = state.teacherEliteOn ? '👑 Teacher Elite: ON' : '👑 Teacher Elite: OFF';
  if(!box) return;
  if(!isTeacherJrMode()){
    box.style.display = 'none';
    updateTeacherControlUI();
    return;
  }
  const nextTheme = getWeekThemeForDisplay(Math.min(TOTAL_WEEKS, Math.max(1, (state.week || 0) + (state.choiceMade ? 1 : 0))));
  box.style.display = '';
  if(state.teacherControlOn){
    box.innerHTML = `<b>Teacher control active.</b> You can choose what gets taught for each week. Guided Elite themes pause while manual choices are in charge.<br><div style="margin-top:8px"><b>Current/next focus:</b> ${nextTheme.label}</div><div style="margin-top:8px">${getCoverageStatusMarkup()}</div>`;
  } else {
    box.innerHTML = state.teacherEliteOn
      ? `<b>Elite plan active.</b> 12 weeks stay simple, but each week now follows a theme and coverage tracker.<br><div style="margin-top:8px"><b>Next theme:</b> ${nextTheme.label}</div><div style="margin-top:8px">${getCoverageStatusMarkup()}</div>`
      : `<b>Elite plan off.</b> Weeks use a fully random unseen-scenario mix for replayability.`;
  }
  updateTeacherControlUI();
}
function toggleTeacherElite(){
  ensureEliteState();
  state.teacherEliteOn = !state.teacherEliteOn;
  if(state.teacherEliteOn) state.teacherControlOn = false;
  state.scenarioPlan = {};
  state.scenarioCoverage = { usedScenarioIds: [], usedTags: [], planMode: state.teacherEliteOn ? 'elite' : 'random' };
  updateTeacherEliteUI();
  saveGameProgress(true);
  const theme = getWeekThemeForDisplay(Math.max(1, state.week || 1));
  showModal('👑', state.teacherEliteOn ? 'Teacher Elite ON' : 'Teacher Elite OFF', state.teacherEliteOn ? `Elite mode is on. The game stays 12 weeks and simple, but now each week follows a theme like <b>${theme.label}</b> and the game tracks coverage so the class sees a fuller mix.` : 'Elite mode is off. The game now uses the random replay mix again.', 'Got it!', closeModal);
}
function toggleTeacherControl(){
  ensureEliteState();
  state.teacherControlOn = !state.teacherControlOn;
  if(state.teacherControlOn) state.teacherEliteOn = false;
  if(!state.teacherControlOn) state.teacherManualWeekThemes = {};
  state.scenarioPlan = {};
  state.scenarioCoverage = { usedScenarioIds: [], usedTags: [], planMode: state.teacherControlOn ? 'manual' : (state.teacherEliteOn ? 'elite' : 'random') };
  updateTeacherEliteUI();
  saveGameProgress(true);
  showModal('🎛️', state.teacherControlOn ? 'Teacher Control ON' : 'Teacher Control OFF', state.teacherControlOn ? 'Teacher control is on. Pick a topic button to choose what the class learns for the current or next week.' : 'Teacher control is off. The game goes back to guided Elite or random mode.', 'Got it!', closeModal);
}
function setTeacherWeekControl(tag){
  ensureEliteState();
  if(!state.teacherControlOn){
    state.teacherControlOn = true;
    state.teacherEliteOn = false;
  }
  const targetWeek = getTeacherControlTargetWeek();
  state.teacherManualWeekThemes[String(targetWeek)] = tag;
  state.scenarioPlan = {};
  state.scenarioCoverage = { usedScenarioIds: [], usedTags: [], planMode: 'manual' };
  buildScenarioPlanIfNeeded();
  const selectedTheme = tag === 'random' ? null : getTeacherControlledThemeForWeek(targetWeek);
  updateTeacherEliteUI();
  saveGameProgress(true);
  if(state.gameActive && targetWeek === state.week && !state.choiceMade){
    const scenario = getScenarioForWeek(targetWeek);
    if(scenario){
      document.getElementById('choice-card').style.display = 'none';
      document.getElementById('next-btn').disabled = true;
      document.getElementById('spend-section').style.display = 'none';
      const theme = getWeekThemeForDisplay(targetWeek);
      document.getElementById('teacher-info').textContent = `Week ${targetWeek}: ${theme.label} · ${scenario.scenario} | FL Standards: Earning (#1), Saving (#3), Spending (#2), Sharing`;
      updateTeacherWeekPrompt(scenario);
      if(scenario.skipChoice){
        showModal(scenario.emoji, `Week ${targetWeek} Updated`, `${scenario.scenario}<br><br>${scenario.question || 'This week is now set by teacher control.'}`, 'Got it!', () => {
          showSpendSection();
        ...
        showChoiceCard(scenario);
      }
    }
  }
  const body = tag === 'random'
    ? `Week ${targetWeek} will use a random mix again.`
    : `Week ${targetWeek} is now set to <b>${selectedTheme ? selectedTheme.label : tag}</b>. The class will see a matching scenario.`;
  showModal('🧭', 'Week Focus Updated', body, 'Got it!', closeModal);
}
// Spend shop items// Spend shop items (same for both jobs)
const SHOP_ITEMS = [
...
function refreshGameUIAfterLoad(){
  ...
  if(state.gameActive || state.finalReached){
    ...
    if(!state.finalReached){
      const scenario = getScenarioForWeek(Math.max(1, state.week || 1));
...
function returnToDifficultySelect(event){
...
function loadWeekScenario(week) {
  ...
  const scenario = getScenarioForWeek(week);
  ...
  const theme = getWeekThemeForDisplay(week);
  // Update teacher info
  document.getElementById('teacher-info').textContent = state.teacherEliteOn
    ? `Week ${week}: ${theme.label} · ${scenario.scenario} | FL Standards: Earning (#1), Saving (#3), Spending (#2), Sharing`
...
function makeChoice(opt, scenario, btn) {
  ...
  const historyCount = (state.reflectionHistory || []).filter(item => item.type === reflectionType).length;
  const leadQuestion = pack.reflection[historyCount % pack.reflection.length];
...
(function(){
  ...
  if(teacherOn){
    updateTeacherEliteUI();
    renderTeacherReflectionBox('<div><b>Teacher mode ready.</b><br>Click a question in the reflection area to show it to the class.</div>');
  }
})();
